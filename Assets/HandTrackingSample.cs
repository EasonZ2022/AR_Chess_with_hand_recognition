    ...
    void Update()
    {
        if (hand_recog_mode_on && landmarkResult != null && landmarkResult.score > 0.2f)
        {
            ...
            if (!first_frame && PlaceBoard.board_placed)
            {
                ...
                new_cursor.transform.Translate((curr_y - prev_y) * (-1.0f), 0f, (curr_x - prev_x) * (-1.0f), Space.Self);
                ...
                Display1.text = string.Format("pos:{0},{1} dis:{2}", x1, y1, disSquare);
                if (disSquare < tipThreshold)
                {
                    Display1.text = string.Format("Good!!! dis:{0}", disSquare);
